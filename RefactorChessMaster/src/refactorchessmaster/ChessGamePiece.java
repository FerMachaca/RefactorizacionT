...
public abstract class ChessGamePiece {
    ...
    protected ArrayList<String> calculateWestMoves(
        ...
        if (isPieceOnScreen()) {
            for (int i = pieceColumn - 1; i >= 0 && count < numMoves; i--) {
                moves = movePiece(board, pieceRow, i, moves);
                if(moves!=null){
                    count++;
                }else{
                    break;
    ...
    public ArrayList<String> movePiece(ChessGameBoard board, int left, int ri, ArrayList<String> moves) {
        if ((board.getCell(left, ri).getPieceOnSquare() == null || isEnemy(board, left, ri))) {
            moves.add(left + "," + ri);
            if (isEnemy(board, left, ri)) {
                return null;
            } else {
                return moves;
            }
